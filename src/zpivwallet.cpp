...
CzBNXXWallet::CzBNXXWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBNXXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBNXXSeed_deprecated()) {
                    LogPrintf("%s: Updated zBNXX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzBNXXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBNXXCount(nCountLastUsed);
    else if (!walletdb.ReadZBNXXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBNXXWallet::Lock()
{
    seedMaster = 0;
}

void CzBNXXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBNXXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBNXX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBNXXWallet::LoadMintPoolFromDB()
{
...
void CzBNXXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBNXXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBNXXWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzBNXXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBNXX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzBNXXWallet::SeedToZBNXX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBNXXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBNXXWallet::GenerateDeterministicZBNXX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBNXXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBNXXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
