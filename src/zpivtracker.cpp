...
CzBNXXTracker::CzBNXXTracker(std::string strWalletFile)
{
...
CzBNXXTracker::~CzBNXXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBNXXTracker::Init()
{
...
bool CzBNXXTracker::Archive(CMintMeta& meta)
{
...
bool CzBNXXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBNXXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBNXXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBNXXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBNXXTracker::GetSerialHashes()
{
...
CAmount CzBNXXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBNXXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBNXXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBNXXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBNXXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBNXXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBNXXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBNXXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBNXXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBNXXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBNXXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBNXXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBNXXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBNXXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBNXXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBNXXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBNXXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
