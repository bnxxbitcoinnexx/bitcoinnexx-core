...
void test_ge(void) {
     ...
     *
     * When the enbitcoinnexxrphism code is compiled in, p5 = lambda*p1 and p6 = lambda^2*p1 are added as well.
     */
...
void test_add_neg_y_diff_x(void) {
     ...
     *
     * # enbitcoinnexxrphism values (lambda is 1^{1/3} in N, beta is 1^{1/3} in F)
     * x = polygen(N)
     * lam  = (1 - x^3).roots()[1][0]
     *
     * # random "bad pair"
...
void test_constant_wnaf(const secp256k1_scalar *number, int w) {
    ...
    /* With USE_ENBNXXRPHISM on we only consider 128-bit numbers */
#ifdef USE_ENBNXXRPHISM
    for (i = 0; i < 16; ++i) {
...
void run_enbitcoinnexxrphism_tests(void) {
    test_scalar_split();
}
#endif
...
int main(int argc, char **argv) {
    ...
    /* enbitcoinnexxrphism tests */
#ifdef USE_ENBNXXRPHISM
    run_enbitcoinnexxrphism_tests();
#endif
