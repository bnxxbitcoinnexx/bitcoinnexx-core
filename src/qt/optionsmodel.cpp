...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BNXX);
    ...
    if (!settings.contains("nAnonymizeBitcoinNexxAmount"))
        settings.setValue("nAnonymizeBitcoinNexxAmount", 1000);

    nAnonymizeBitcoinNexxAmount = settings.value("nAnonymizeBitcoinNexxAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBitcoinNexxAmount"))
        SoftSetArg("-anonymizebitcoinnexxamount", settings.value("nAnonymizeBitcoinNexxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bitcoinnexx.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBitcoinNexxAmount:
            return QVariant(nAnonymizeBitcoinNexxAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBitcoinNexxAmount:
            nAnonymizeBitcoinNexxAmount = value.toInt();
            settings.setValue("nAnonymizeBitcoinNexxAmount", nAnonymizeBitcoinNexxAmount);
            emit anonymizeBitcoinNexxAmountChanged(nAnonymizeBitcoinNexxAmount);
            break;
        case CoinControlFeatures:
