...
#include "ui_zbnxxcontroldialog.h"
...
std::set<std::string> ZBnxxControlDialog::setSelectedMints;
std::set<CMintMeta> ZBnxxControlDialog::setMints;

ZBnxxControlDialog::ZBnxxControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZBnxxControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZBnxxControlDialog::~ZBnxxControlDialog()
{
    delete ui;
}

void ZBnxxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBnxxControlDialog::updateList()
{
...
void ZBnxxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBnxxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBnxx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBnxxControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBnxxControlDialog::GetSelectedMints()
{
...
void ZBnxxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
