...
#if defined(NDEBUG)
#error "BNXX cannot be compiled without assertions."
#endif
...
{
    //Check to see if the zBNXX is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zBNXX spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zBNXX without the correct spend type. txid=%s", __func__,
                tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zBNXX spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zBNXX spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zBNXX spend tx %s already in block %d",
            ...
            // Check that zBNXX mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else
    {
        nSubsidy = 10 * COIN;        
        if(nHeight < 8400001) {
            // Subsidy is cut in half every 210,0000 blocks which will occur approximately every 4 years.
            nSubsidy >>= halvings;
        } else
        {
            // After 16 years, the subsidy will be fixed as 1 BNXX
            nSubsidy = 1 * COIN;
        }        
    }    
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZBNXXStake)
{
    int64_t ret = 0;
    ...
    ret = blockValue * 0.80; // 90% of block reward for MN
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from BNXX
...
{
    RenameThread("bitcoinnexx-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZBNXXMinted()
{
...
void RecalculateZBNXXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zBNXX supply
...
bool RecalculateBNXXSupply(int nHeightStart)
{
...
bool UpdateZBNXXSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateBNXXSupply(Params().Zerocoin_StartHeight());
    }

    //Track zBNXX money supply in the block index
    if (!UpdateZBNXXSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zBNXX supply for block=%s height=%d", __func__, block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    ...
    //    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zBitcoinNexxSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert BNXX to zBNXX
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zBNXX spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zBNXX serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zBNXX stake block is less than allowed start height", __func__);

    if (CZBitcoinNexxStake* zBNXX = dynamic_cast<CZBitcoinNexxStake*>(stake)) {
        CBlockIndex* pindexFrom = zBNXX->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zBNXX stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zBNXX stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zBNXX->GetValue()));
        if (nChecksum200 != zBNXX->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zBNXX->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZBNXX() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zBNXX fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zBNXX mints and %d zBNXX spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BNXX: We use certain sporks during IBD, so check to see if they are
