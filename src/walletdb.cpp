...
void ThreadFlushWalletDB(const string& strFile)
{
    ...
    RenameThread("bitcoinnexx-wallet");
...
bool CWalletDB::WriteZBNXXSeed(const uint256& hashSeed, const vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZBNXXSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZBNXXSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZBNXXSeed_deprecated()
{
    return Erase(string("dzs"));
}

bool CWalletDB::ReadZBNXXSeed(const uint256& hashSeed, vector<unsigned char>& seed)
{
    return Read(make_pair(string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZBNXXSeed_deprecated(uint256& seed)
{
    return Read(string("dzs"), seed);
}

bool CWalletDB::WriteZBNXXCount(const uint32_t& nCount)
{
    return Write(string("dzc"), nCount);
}

bool CWalletDB::ReadZBNXXCount(uint32_t& nCount)
{
