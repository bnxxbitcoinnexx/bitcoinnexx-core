...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BNXX at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBNXX send
enum ZerocoinSpendStatus {
    ZBNXX_SPEND_OKAY = 0,                            // No error
    ZBNXX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBNXX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBNXX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBNXX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBNXX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBNXX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBNXX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBNXX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBNXX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBNXX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBNXX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBNXX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBNXX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBNXX_SPENT_USED_ZBNXX = 14,                      // Coin has already been spend
    ZBNXX_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZBNXX_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZBitcoinNexxBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZBNXXOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzBNXXWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzBNXXTracker> zpivTracker;
    ...
    void setZWallet(CzBNXXWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzBNXXTracker>(new CzBNXXTracker(strWalletFile));
    }

    CzBNXXWallet* getZWallet() { return zwalletMain; }
    ...
    void setZBitcoinNexxAutoBackups(bool fEnabled)
    {
    ...
    /** zBNXX reset */
    boost::signals2::signal<void()> NotifyzBNXXReset;
