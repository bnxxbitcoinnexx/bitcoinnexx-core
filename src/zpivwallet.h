...
#ifndef BNXX_ZBNXXWALLET_H
#define BNXX_ZBNXXWALLET_H
...
public:
    CzBNXXWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZBNXX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZBNXX(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //BNXX_ZBNXXWALLET_H
